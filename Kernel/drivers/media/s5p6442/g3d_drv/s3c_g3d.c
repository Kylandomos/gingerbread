...
static DEFINE_MUTEX(mem_sfr_mmap_lock);
...
//			printk("\n\n\n#########G3d -timeout #######\n\n");
	...
	case S3C_3D_MAP_FIMG_SFR_ADDR:
		mutex_lock(&mem_sfr_mmap_lock);
		if(copy_from_user(&param, (struct s3c_3d_mem_alloc *)arg, sizeof(struct s3c_3d_mem_alloc))){
			mutex_unlock(&mem_sfr_mmap_lock);
			return -EFAULT;
		}

		//printk("param value size= %x phy=  %x \n",param.size ,param.phy_addr );
		flag = GET_FIMG_ADDR;
		param.vir_addr = do_mmap(file, 0, param.size, PROT_READ|PROT_WRITE, MAP_SHARED, param.phy_addr);
		
		//printk("param value size= %x  virt= %x \n",param.size ,param.vir_addr );
