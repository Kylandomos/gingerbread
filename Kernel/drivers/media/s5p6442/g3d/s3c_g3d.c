...
static int s3c_g3d_unlock(struct g3d_context *ctx)
{
	...
	//SEB pm_runtime_mark_last_busy(data->dev);
	//SEB pm_runtime_put_autosuspend(data->dev);
...
static int __devexit s3c_g3d_remove(struct platform_device *pdev)
{
	...
	//SEB pm_runtime_suspend(&pdev->dev);
	//SBE pm_runtime_disable(&pdev->dev);
 ...
 * Module operations
 */
static char banner[] __initdata = KERN_INFO "S3C G3D OpenFIMG Driver\n";
